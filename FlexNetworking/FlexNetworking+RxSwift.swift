...
extension FlexNetworking.Rx {
    ...
    ///
    public func runRequestWithoutHooks(session: URLSession = .shared, path: String, method: String, body: RequestBody?, headers: [String: String] = [:]) -> Single<Response> {
    ...
    /// When the returned Single is disposed, it cancels the task corresponding to the request initiated by the observable.
    ///
    public func runRequest(session: URLSession = .shared, path: String, method: String, body: RequestBody?, headers: [String: String] = [:]) -> Single<Response> {
        ...
        for hook in self.flex.postRequestHooks {
            lastResponse = lastResponse.flatMap { (response, originalRequestParameters, shouldContinue) in
                ...
                case .makeNewRequest(let (session, path, method, body, headers)):
                    let nextResponse = self.runRequestWithoutHooks(session: session, path: path, method: method, body: body, headers: headers)
